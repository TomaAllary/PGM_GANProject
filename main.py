...
def discriminator_grid_search(data_manipulator: DataManipulator):
    ...
    for batch_size in [32, ]:
        for lr in [1e-3,]:
            ...
            train_discriminator_only(data_manipulator, generator_model, discriminator_model, param_str=f"Lr={lr}, reg=0.001, batch_size={batch_size}")

def train_discriminator_only(data_manipulator: DataManipulator, generator, discriminator, param_str=None):

    discriminator.set_hyperparameter('learning_rate', 7.5)
    discr = DiscriminatorFramework(generator, discriminator, data_manipulator.x_train, data_manipulator.x_validation)

    discriminator_metrics_per_epoch, loss_curve, acc_curve = discr.train(30)
...
def main():
    ...
    # train_discriminator_only(data_manipulator, generator_model, discriminator_model)
